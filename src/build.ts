...
export async function build(
  opts: BuildOptions & { config: NuxtBuilderConfig }
): Promise<BuilderOutput> {
  const { files, entrypoint, workPath, config = {}, meta = {} } = opts;
  // ---------------- Debugging context --------------
  consola.log(
    "Running with @nuxt/vercel-builder version",
  ...
  const usesTypescript =
    (pkg.devDependencies &&
      Object.keys(pkg.devDependencies).includes("@nuxt/typescript-build")) ||
    (pkg.dependencies &&
      Object.keys(pkg.dependencies).includes("@nuxt/typescript"));
  const needsTypescriptBuild =
    getNuxtConfigName(entrypointPath) === "nuxt.config.ts";
  ...
  if (process.env.VERCEL_ANALYTICS_ID) {
    consola.log(
      "Vercel Analytics Detected. Adding @nuxtjs/web-vitals to .nuxtrc"
    );
    updaterc(
  ...
  if (needsTypescriptBuild) {
    ...
    compiledTypescriptFiles = await compileTypescriptBuildFiles({
      rootDir: entrypointPath,
      spawnOpts,
      tscOptions,
    });
  ...
  const buildDir = nuxtConfigFile.buildDir
    ? path.relative(entrypointPath, nuxtConfigFile.buildDir)
    : ".nuxt";
  const srcDir = nuxtConfigFile.srcDir
    ? path.relative(entrypointPath, nuxtConfigFile.srcDir)
    : ".";
  ...
  const usesServerMiddleware =
    config.internalServer !== undefined
      ...
      : !!nuxtConfigFile.serverMiddleware;
